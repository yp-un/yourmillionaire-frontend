...
import { Badge, Button, Card, CardContent, CardHeader, CardTitle, Separator } from "@millionaire/ui";
...
export default function Home() {
  return (
    <main className="min-h-screen bg-white">
      <section className="relative isolate overflow-hidden border-b">
        ...
        <header className="container flex h-16 items-center justify-between">
          ...
          <Button asChild size="sm">
            <Link href="https://dashboard.domain.com">
              대시보드
        ...
        <div className="container flex min-h-[76svh] flex-col justify-center pb-14 pt-12">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 max-w-xl text-balance text-lg leading-8 text-slate-700">
              YourMillionaire는 회계 소프트웨어를 처음 보는 창업자가 은행 계좌를 연결하고, 수집된 거래가 분개로 정리되는 과정을 확인하는 대시보드입니다.
            </p>
      ...
      <section id="persona" className="container grid gap-8 py-16 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <Badge variant="outline">Persona</Badge>
          <h2 className="mt-4 text-3xl font-semibold tracking-normal">회계 SW를 한 번도 써본 적 없는 공동창업자</h2>
          <p className="mt-3 text-slate-600">
            민지는 Figma, Notion, Slack은 익숙하지만 회계 화면은 낯섭니다. 법인 통장과 법인카드는 있고, 매달 클라우드 비용과 툴 구독료는 나가지만 재무제표는 읽을 줄 모릅니다.
          </p>
        </div>
      ...
      <section id="workflow" className="container py-16">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 text-3xl font-semibold tracking-normal">회계 용어보다 먼저 보여야 하는 것</h2>
          <p className="mt-3 text-slate-600">
            현재 대시보드는 계좌를 연결하고, 수집된 거래가 어떻게 분개로 정리됐는지 확인하는 흐름에 집중합니다.
          </p>
        </div>
          ...
          {workflow.map((item) => {
            ...
            return (
              <Card key={item.title}>
                ...
                <CardContent className="text-sm leading-6 text-slate-600">{item.body}</CardContent>
              </Card>
            );
          })}
        </div>
      ...
      <section id="features" className="border-y bg-slate-50">
        <div className="container grid gap-8 py-16 lg:grid-cols-[1fr_0.9fr]">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-semibold tracking-normal">민지가 실제로 누르게 될 기능</h2>
          ...
          <div className="grid gap-2">
            {currentFeatures.map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-lg border bg-white p-3 text-sm">
      ...
      <section id="security" className="container grid gap-6 py-16 md:grid-cols-2">
        <Card>
          <CardHeader>
            <LockKeyhole className="mb-3 size-6 text-primary" aria-hidden="true" />
            <CardTitle>은행 인증은 연결에만 사용</CardTitle>
          </CardHeader>
          <CardContent className="text-sm leading-6 text-slate-600">
            은행 비밀번호는 CODEF 연결 생성에만 사용되며, 프론트엔드 화면에는 연결 결과로 받은 계좌 목록만 표시합니다.
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
