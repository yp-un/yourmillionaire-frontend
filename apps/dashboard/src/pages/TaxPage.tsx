...
const taxStrategyScenarios: Array<{
	label: string;
	value: TaxStrategyScenario;
}> = [
...
export function TaxPage() {
	...
	const [strategyScenario, setStrategyScenario] = useState<TaxStrategyScenario>(
		"applicable_benefits",
	);
	...
	async function loadTaxData() {
		...
		try {
			const [nextProfile, nextFilings, nextWithholding, nextInvoices] =
				await Promise.all([
					api.getCorporationProfile(selectedTenantId).catch((profileError) => {
						if (
							profileError instanceof YmApiError &&
					...
					api.getTaxInvoices(selectedTenantId, {
						from: invoiceFrom,
						to: invoiceTo,
					}),
				]);
			...
			setError(
				toTaxErrorMessage(loadError, "세무 데이터를 불러오지 못했습니다."),
			);
		} finally {
			setLoading(false);
		}
	...
	async function handleSaveProfile() {
		...
		try {
			const nextProfile = await api.upsertCorporationProfile(
				selectedTenantId,
				formToProfileRequest(profileForm),
			);
			setProfile(nextProfile);
			setProfileForm(profileToForm(nextProfile));
		} catch (saveError) {
			setError(
				toTaxErrorMessage(saveError, "회사 프로필을 저장하지 못했습니다."),
			);
		} finally {
			setSaving(false);
		}
	...
	async function handleRunTaxStrategy() {
		...
		try {
			...
			setError(
				toTaxErrorMessage(runError, "세무 전략 점검을 실행하지 못했습니다."),
			);
		} finally {
		...
		<PageShell>
			...
			<Tabs defaultValue="profile" className="space-y-4">
				<TabsList className="grid h-auto w-full grid-cols-1 rounded-lg min-[420px]:grid-cols-2 sm:rounded-lg md:grid-cols-5 lg:w-auto">
					...
					<TabsTrigger className="rounded-md" value="filings">
						신고
					</TabsTrigger>
					<TabsTrigger className="rounded-md" value="withholding">
						원천세
					</TabsTrigger>
					<TabsTrigger className="rounded-md" value="invoices">
						세금계산서
					</TabsTrigger>
					<TabsTrigger className="rounded-md" value="agent">
						AI 세무
					</TabsTrigger>
				</TabsList>
					...
					<Card>
						...
						<CardContent className="grid gap-4 lg:grid-cols-3">
							<Field label="설립일">
								<Input
									type="date"
									value={profileForm.foundedOn}
									onChange={(event) =>
										updateProfileForm("foundedOn", event.target.value)
									}
								/>
							</Field>
							<Field label="본점 소재지">
								<Select
									...
									onValueChange={(value) =>
										updateProfileForm("regionCode", value)
									}
								>
									<SelectTrigger>
									...
									<SelectContent>
										{profileForm.regionCode &&
										!regionOptions.some(
											(option) => option.value === profileForm.regionCode,
										) ? (
											<SelectItem value={profileForm.regionCode}>
												저장된 지역
											</SelectItem>
										) : null}
							...
							<Field label="업종">
								<Select
									value={profileForm.industryCode}
									onValueChange={(value) =>
										updateProfileForm("industryCode", value)
									}
								>
									<SelectTrigger>
									...
									<SelectContent>
										{profileForm.industryCode &&
										!industryOptions.some(
											(option) => option.value === profileForm.industryCode,
										) ? (
											<SelectItem value={profileForm.industryCode}>
												저장된 업종
											</SelectItem>
										) : null}
							...
							<Field label="원천세 신고 주기">
								<Select
									value={profileForm.withholdingCadence}
									onValueChange={(value) =>
										updateProfileForm(
											"withholdingCadence",
											value as ProfileForm["withholdingCadence"],
										)
									}
								>
									<SelectTrigger>
							...
							<Field label="회계연도 시작월">
								<Select
									value={profileForm.fiscalYearStartMonth}
									onValueChange={(value) =>
										updateProfileForm("fiscalYearStartMonth", value)
									}
								>
									<SelectTrigger>
							...
							<ToggleField
								label="청년창업자"
								checked={profileForm.isYouthFounder}
								onChange={(value) => updateProfileForm("isYouthFounder", value)}
							/>
							<ToggleField
								label="벤처 인증"
								checked={profileForm.isVentureCertified}
								onChange={(value) =>
									updateProfileForm("isVentureCertified", value)
								}
							/>
							<ToggleField
								label="외부감사 대상"
								checked={profileForm.isExternalAudit}
								onChange={(value) =>
									updateProfileForm("isExternalAudit", value)
								}
							/>
						...
						<CardContent className="space-y-3">
							{filings.length === 0 ? (
								...
								filings.map((filing) => (
									<div
										key={filing.id}
										className="flex flex-col gap-2 rounded-md border p-3 text-sm lg:flex-row lg:items-center lg:justify-between"
									>
										<div>
											<p className="font-semibold">
												{filingKindLabel(filing.kind)}
											</p>
										...
										<div className="text-left lg:text-right">
											<p className="font-medium">
												영업일 기준 {filing.businessDueDate}
											</p>
											<p className="text-muted-foreground">
												법정기한 {filing.statutoryDueDate}
											</p>
										</div>
									</div>
								))
						...
						<CardContent className="space-y-3">
							{withholding.length === 0 ? (
								...
								withholding.map((item) => (
									<div
										key={item.id}
										className="flex flex-col gap-3 rounded-md border p-3 text-sm lg:flex-row lg:items-center lg:justify-between"
									>
										<div>
											<p className="font-semibold">{item.payeeLabel}</p>
											<p className="text-muted-foreground">
												{item.incomeType} · 지급일 {item.paymentDate}
											</p>
										</div>
										<div className="flex items-center gap-3">
											<span className="font-medium number-tabular">
												{formatCurrency(item.incomeTax + item.localIncomeTax)}
											</span>
											<Button
												size="sm"
												variant="outline"
												onClick={() => void handleFileWithholding(item.id)}
											>
												신고 처리
											</Button>
										</div>
						...
						<CardHeader>
							<CardTitle className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
								...
								<div className="ym-date-grid lg:min-w-[26rem]">
									...
									/>
									<Button variant="outline" onClick={() => void loadTaxData()}>
										조회
									</Button>
								</div>
							</CardTitle>
						...
						<CardContent className="space-y-3">
							{invoices.length === 0 ? (
								...
								invoices.map((invoice) => (
									<div
										key={invoice.id}
										className="grid gap-2 rounded-md border p-3 text-sm lg:grid-cols-[1fr_auto_auto] lg:items-center"
									>
										<div>
											<p className="font-semibold">
												{invoice.direction === "SALE" ? "매출" : "매입"} ·{" "}
												{invoice.writtenDate}
											</p>
										...
										<Badge
											variant={invoice.isDeductible ? "success" : "warning"}
										>
											{invoice.isDeductible ? "공제" : "불공제"}
										</Badge>
									</div>
								))
							)}
						...
						<CardContent className="space-y-4">
							<div className="grid gap-3 sm:grid-cols-[1fr_auto]">
								<Select
									...
									onValueChange={(value) =>
										setStrategyScenario(value as TaxStrategyScenario)
									}
								>
									<SelectTrigger>
										<SelectValue />
							...
							{strategyError ? (
								<Notice tone="danger">
									{strategyError.reason ?? "전략 점검 중 오류가 발생했습니다."}
								</Notice>
							) : null}
	...
	function updateProfileForm<Key extends keyof ProfileForm>(
		key: Key,
...
function EmptyText({ children }: { children: ReactNode }) {
	return (
		<p className="ym-panel p-4 text-sm text-muted-foreground">{children}</p>
	);
}
...
function getStrategyPendingMessage(events: TaxStrategyEvent[]) {
	...
	if (latestUsefulEvent.type === "started") {
		return "세무 점검을 준비하고 있습니다.";
	}

	if (latestUsefulEvent.type === "context_ready") {
		return "회사 데이터와 신고 일정을 확인하겠습니다.";
	}
...
function getToolCallMessage(event: TaxStrategyEvent) {
	const input = isRecord(event.input) ? event.input : {};

	if (event.name === "search_tax_law") {
	...
	if (event.name === "compute_penalty_scenario") {
		return "가산세 부담액을 계산하고 있습니다.";
	}

	if (event.name === "check_benefit_eligibility") {
		return "적용 가능한 세제 혜택 요건을 확인하고 있습니다.";
	}

	if (event.name === "get_filing_draft_detail") {
		return "신고 초안과 관련 금액을 확인하고 있습니다.";
	}
...
function profileToForm(profile: CorporationProfile | null): ProfileForm {
	...
	return {
		...
		priorYearCorpTax: profile.priorYearCorpTax
			? String(profile.priorYearCorpTax)
			: "",
		priorYearRevenue: profile.priorYearRevenue
			? String(profile.priorYearRevenue)
			: "",
...
function isSameProfileForm(left: ProfileForm, right: ProfileForm) {
	return (Object.keys(emptyProfileForm) as Array<keyof ProfileForm>).every(
...
function formToProfileRequest(form: ProfileForm) {
	return {
		...
		priorYearCorpTax: form.priorYearCorpTax
			? Number(form.priorYearCorpTax)
			: undefined,
		priorYearRevenue: form.priorYearRevenue
			? Number(form.priorYearRevenue)
			: undefined,
