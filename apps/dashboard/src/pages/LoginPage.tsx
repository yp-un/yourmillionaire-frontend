...
export function LoginPage() {
  ...
  return (
    <main className="flex min-h-screen bg-[#f7fafc] text-[#111827]">
      <section className="hidden min-h-screen flex-1 border-r border-slate-200 bg-white p-10 lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Bank Journal Beta</p>
          </div>
        </div>
      ...
      <section className="flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
          <div className="mb-8">
            <div className="mb-4 flex size-12 items-center justify-center rounded-lg bg-primary text-white lg:hidden">
              <Landmark className="size-6" aria-hidden="true" />
            </div>
            <h2 className="text-2xl font-semibold">로그인</h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">Google 계정으로 로그인해 워크스페이스를 불러옵니다.</p>
          </div>
          ...
          <p className="mt-5 text-xs leading-5 text-slate-500">로그인 세션이 만료되면 자동 갱신을 시도하고, 실패 시 다시 로그인합니다.</p>
        </div>
      </section>
    </main>
  );
